...
[现在试试](https://dice.weizaima.com/) (官网首页，点在线试用)
...
首先，找到压缩包再解压一份海豹，现在你有“海豹1”和“海豹2”两个目录。
...
可以，见 [Linux 部署](#Linux 部署) 和 [Mac部署](#Mac部署)。
...
格式: `.ch save <角色名>` // 保存角色，角色名省略则为当前昵称

格式: `.ch load <角色名>` // 加载角色，角色名省略则为当前昵称

格式: `.ch list` // 列出当前角色

格式: `.ch del/rm <角色名>` // 删除角色
...
这个指令中save的机制类似于传统RPG的存档。并不会自动绑卡！！！
...
后续会补全绑卡功能，以及pc new指令（创建空白卡，并自动绑卡）
...
```bash
curl https://dice.weizaima.com/s/linux.sh | bash
```
...
注意！如果二次启动海豹，最后还是右键打开终端窗口，然后运行指令中第二句。直接双击运行可能会有权限不足的问题。
