...
这种情况可以删除 data/helpdoc 来进行提速，代价是[.find/.查询 查询指令](#find查询-查询指令)出不来东西。
...
感谢社区，我们现在有了[一键qsign](https://github.com/rhwong/unidbg-fetch-qsign-onekey)。

> 下面的话我是直接引用了一键qsign里的说明

点开以后删掉文件夹里的 `go-cqhttp.bat`及 `go-cqhttp_windows_386.exe`，然后运行里面的 `Start_Qsign.bat` 启动qsign，按照提示依次键入 `txlib_version` 参数、设定 `host`、`port`、`key`的值。 (👈如果不知道这些是干什么的，请直接依次按下Enter）
...
你在登录账号的时候会看到这样一个界面。![qsign-1.png](static/manual.assets/qsign-1.png)点击下面的“签名服务”一栏的“简易配置”，然后会变成这样：

![qsign-2.png](static/manual.assets/qsign-2.png)
...
你没有特殊设置的话qsign的url默认为`http://localhost:13579`，mac和win的key没有特殊设置则会在你启动qsign时随机生成一个并显示在qsign的命令行里，而linux的默认就是114514
...
2. 使用正则匹配时，直接在要匹配的文本一栏中写入正则表达式即可。正则表达式可以参照[https://www.runoob.com/regexp/regexp-tutorial.html](https://www.runoob.com/regexp/regexp-tutorial.html) 在这里给出一个示例： `^测试(.+)吗$` 这一正则表达式将会匹配所有开头是 `测试` 而结尾是 `吗` 的回复，但中间如果存在换行则不匹配。如果没有 `^` `$` 两个符号，则会是同时包含 `测试`  `吗`的任何发言。此乃贪婪匹配是也，所以请注意做好限定。
...
海豹已经在github开源，欢迎有实力或有毅力或感兴趣的用户参与贡献。
链接：[github - sealdice/sealdice-core](https://github.com/sealdice/sealdice-core)
