...
##### (1)文本匹配
这应该是最好理解的一个条件了吧。你输入什么，海豹接收到以后就会返回你给定的内容。注意，在这里，是可以嵌入CQ码的，任何特殊消息类型都会被解析为CQ码。
[CQ码列表](https://docs.go-cqhttp.org/cqcode)
文本匹配的方式中又分为七种，其中简单的有“精确匹配”“包含文本”“不含文本”“前缀匹配”“后缀匹配”，“模糊匹配”则是按照一定的算法进行匹配（很玄学），“正则匹配”就是按照正则表达式进行匹配。
##### (2)表达式为真
这一项大概率作为限制条件出现。在自定义回复中，触发条件是“必须同时满足”。举个例子：如果我想让海豹只回复特定的人，我就可以这么写:
`$t账户ID=QQ号`
但是这里更建议的是使用[if](#if语法)来进行区分。
##### (3)文本长度
常用于限制条件。当你选择了他，下面会出现“大于等于”/“小于等于” 字数，很好理解。
...
就这？没有更难的？
别急，这就到了。
在回复文本中，可以调用一些[变量](#变量机制)，也可以嵌入[脚本语言](#内置脚本语言)。
###### 调用变量
对海豹输入`.text {$t玩家}进行测试`
...
###### 执行块
执行块是指{% %}这一部分。由{% %}括起来的部分会被作为代码执行，然后输出最后一个语句的结果。如果想要输出字符串，则应该用反引号/双引号/单引号括上（例：形如{% \$t1="114514" %}的式子会输出114514）。若想在结果中调用变量，请用反引号。目前海豹只支持在反引号中调用变量。
执行块中的两个语句之间要使用分号(;)隔开。
...
> 在V1.1.0 v20221231后，海豹已经支持了Discord和Kook(开黑啦)，你可以在添加账号时的**账号类型**里看到他们。感谢Szzrain大佬提交的PR。
