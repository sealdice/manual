...
[GitHub - sealdice/qsign-gocq-deploy-pack: All-in-One (hopefully) deploy script for QSign and GoCQ service](https://github.com/sealdice/qsign-gocq-deploy-pack)

clone下来直接运行`.\deploy.sh`。

如果不知道clone是啥，建议别用linux了，天涯何处无芳草:)
...
你没有特殊设置的话qsign的url默认http://localhost:13579，key没有特殊设置则会在你启动qsign时随机生成一个并显示在qsign的命令行里。
